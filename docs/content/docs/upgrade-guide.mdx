...
前者 `createSolanaRpc` 返回一个 `Rpc` 对象，用于向指定的 endpoint 发出 RPC 请求。[在此阅读更多关于 RPC 的信息](/docs/concepts/rpc)。
...
后者 `createSolanaRpcSubscriptions` 返回一个 `RpcSubscriptions` 对象，它允许你订阅 Solana 网络上的事件。[在此阅读更多关于 RPC 订阅的信息](/docs/concepts/rpc-subscriptions)。
...
[你可以在这里阅读更多关于 codecs 的信息](/docs/concepts/codecs)。
...
[查看“兼容客户端”页面](/docs/compatible-clients)以获取与 Kit 兼容的可用程序库的列表。
...
[在此阅读更多关于 signer 的信息](/docs/concepts/signers)。
