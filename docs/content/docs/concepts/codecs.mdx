...
工具包包含一个强大的序列化系统，称为编解码器。无论你是处理帐户数据、指令参数还是自定义二进制布局，编解码器都为你提供将结构化数据转换为字节以及反向转换的工具。

编解码器是可组合的、类型安全的且与环境无关的。它们旨在为跨 Solana 堆栈处理二进制数据提供灵活且一致的基础。

## 安装

编解码器**包含在 `@solana/kit` 库中**，但你也可以使用其独立包安装它们。

```package-install
@solana/codecs
```

请注意，`@solana/codecs` 包本身由几个较小的包组成，每个包都提供一组不同的编解码器助手。以下是包含编解码器的所有包的列表，如果你需要单独安装它们：
...
创建你自己的编解码器的最简单方法是组合你可用的[各种编解码器](#available-codecs)。
...
你可以使用大量的可组合编解码器库，使你能够组合复杂的类型。查看[可用编解码器](#available-codecs)部分以获取更多信息。如果你需要无法从现有编解码器组合的自定义编解码器，你可以随时创建自己的编解码器，正如我们将在下面的[“创建自定义编解码器”](#creating-custom-codecs)部分中看到的那样。
...
正如你所看到的，第一个类型参数更宽松，因为它接受数字或大整数，而第二个类型参数仅接受大整数。这是因为在 _编码_ u64 数字时，你可以为了方便而提供 `bigint` 或 `number`。但是，当你解码 u64 数字时，你将始终获得一个 `bigint`，因为并非所有 u64 值都可以容纳在 JavaScript `number` 类型中。
...
这是另一个使用具有默认值的对象的示例。你可以在下面阅读有关 [transformCodec](#transform-codec) 助手的更多信息。
...
如果组合编解码器对你来说还不够，你可以使用 `createCodec` 函数来实现你自己的编解码器逻辑。此函数需要一个带有 `read` 和 `write` 函数的对象，该对象告诉我们如何从现有字节数组读取和写入。
...
你还可以使用 `addEncoderSizePrefix` 和 `addDecoderSizePrefix` 函数来分离你的编解码器逻辑，如下所示：
...
你还可以使用 `fixEncoderSize` 和 `fixDecoderSize` 函数来分离你的编解码器逻辑，如下所示：
...
但是，你也可以提供一个 `postOffset` 函数来调整“post-offset”。例如，让我们也将“post-offset”向前推送 2 个字节，以便任何进一步的编解码器都将在我们的 8 字节 `u32` 数字的末尾开始执行其工作。
...
此外，后偏移post-offset函数还提供以下属性： 
 
- `newPreOffset`：应用预偏移函数后的新预偏移量。 
- `postOffset`：原始和未更改的后偏移量。 
 
注意，你也可以决定忽略这些属性以实现绝对偏移量。然而，通常建议使用相对偏移量，因为它们在与其他编解码器组合时不会破坏编解码器。
...
请注意 `padLeftCodec` 函数是 `offsetCodec` 和 `resizeCodec` 函数的一个简单封装。对于更复杂的填充策略，你可能需要直接使用 [offsetCodec](#offset-codec) 和 [resizeCodec](#resize-codec) 函数。
...
请注意 `padRightCodec` 函数是 `offsetCodec` 和 `resizeCodec` 函数的一个简单封装。对于更复杂的填充策略，你可能需要直接使用 [offsetCodec](#offset-codec) 和 [resizeCodec](#resize-codec) 函数。
...
`resizeEncoder` 和 `resizeDecoder` 函数也可用于将你的 codec 逻辑拆分为可 tree-shake 的函数。
...
请注意，数字 codec 已经可以通过其 `endian` 选项为你执行此操作。
...
请注意，映射解码类型的第二个函数是可选的。这意味着，你可以省略它以简单地更新或放宽要编码的类型，同时保持解码类型不变。
...
存在类似的辅助程序来映射 `Encoder` 和 `Decoder` 实例，允许你将 codec 逻辑分隔为可 tree-shake 的函数。以下是以这种方式编写的 `getStringU32Codec`。
...
```txt
0XXXXXXX                   <- // 0 到 127 的值（1 个字节）
1XXXXXXX 0XXXXXXX          <- 128 到 16,383 的值（2 个字节）
1XXXXXXX 1XXXXXXX XXXXXXXX <- 16,384 到 4,194,303 的值（3 个字节）
```
...
此外，如果该项是 `FixedSizeCodec`，你可以将 `noneValue` 选项设置为 `"zeroes"`，使返回的可空 codec 也成为 `FixedSizeCodec`。为此，它将用零填充 `null` 值，以匹配现有值的长度。
...
请注意，你可能对可空 code 的 Rust 风格的替代版本感兴趣，该版本可用作 [getOptionCodec](#get-option-codec) 函数。
...
你可以提供一个数字 codec 作为 `getOptionCodec` 函数的 `prefix` 选项，以配置如何储存布尔前缀。
...
此外，如果该项是 `FixedSizeCodec`，你可以将 `noneValue` 选项设置为 `"zeroes"`，以使返回的 Option codec 也成为 `FixedSizeCodec`。为此，它将用零填充 `None` 值，以匹配现有值的长度。
